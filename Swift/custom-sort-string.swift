...
class Solution {
    func customSortString(_ order: String, _ s: String) -> String {
        ...
        for c in s {
          if orderDict.values.contains(c) {
            sDict[c, default: 0] += 1
          } else {
            extraString += String(c)
          }
        }

        for i in 0...orderDict.keys.count {
          if let value = orderDict[i] {
            if sDict.keys.contains(value) {
              orderResult += String(repeating: String(value), count: sDict[value]!)
            }
          }
        }

        return orderResult + extraString
    }
}
